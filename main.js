...
② テクスチャが真っ暗になるのを防ぐルール
   ...
   - 【最重要】クローン時の真っ黒化を防ぐため、クローン元のベーステクスチャは
     必ず完全にロードが完了した状態（init() 内での一括 Promise.all プレロード完了）にすること。
   - ロードした全テクスチャに必ず colorSpace = THREE.SRGBColorSpace を設定すること。
   - 外部設定（levels.json 等）のテクスチャパスを指定する際は、オブジェクトの階層参照ミス
     （config.floor 等の誤指定による undefined 読み込み）がないことを必ず確認すること。
   - 上記を守らないと「同じ画像の多重リクエスト→未ロード状態で描画/クローン→真っ黒」が発生する。
   - GLTF モデルのテクスチャは flipY = false も必須。

③ キャッシュバリア（キャッシュバスター）のルール（デグレード防止）
   - 【最重要】main.js や style.css をわずかでも編集・更新した際は、必ず同時に index.html 内のクエリパラメータ（例: ?v1 → ?v2）をインクリメントし、モバイル端末（iOS Safari / Chrome）の強力なキャッシュによるデグレード（音声バグや高品質音声化の不適用などのバグ再発）を絶対に防ぐこと。
...
④ スマホにおける Web Speech API 音声合成のルール
   ...
   - iOS/Safari ではユーザーがタップした「同期的なコールバック内」で直接 speak() しないと、音声がブロックされるか低品質化する。
   - そのため、setTimeout やタイピング完了後などの非同期のタイミングで最初の speak() を呼び出すことは厳禁。
   - 必ずタップイベントの先頭で unlockAudio() を呼び出して音声ロックを解除し、音声の開始自体はタップされた瞬間に同期的に行い、タイピング演出等は並列でシンクロして走らせること。
   - Siri音声などが未ダウンロードで再生エラーになる場合に備え、Utterance の onerror を監視し、標準日本語音声（Siri以外のKyokoなど）に瞬時にフェイルオーバーさせるロジックを担保すること。
